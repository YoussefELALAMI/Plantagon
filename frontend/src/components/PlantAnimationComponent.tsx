...
const getAnimationUrl = (waterState: string, lightState: string, tempState: string) => {
    if (waterState === 'Optimal' && lightState === 'Optimal' && tempState === 'Optimal') {
      return 'https://lottie.host/embed/c8ae391a-ec7c-4526-abc7-58964e47861e/bpGZ0FRslz.lottie';
    } else if (waterState === 'Dry' || lightState === 'Dark' || tempState === 'Cold' || tempState === 'Hot') {
...
const PlantAnimationComponent: React.FC<PlantAnimationComponentProps> = ({ plantInfo }) => {
  const { waterState, lightState, tempState } = plantState(plantInfo);

  return <iframe src={getAnimationUrl(waterState, lightState, tempState)} title="Plant Animation" />;
};
